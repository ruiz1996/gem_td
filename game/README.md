...
单人建造参考：[带坐标的三进三出迷宫图](docs/solo-maze-plan.md)，附分阶段墙体坐标、核心塔位和当前寻路验证。
...
选中宝石时，相关配方以绿色表示已留材料、蓝色表示当前宝石、黄色表示缺失材料；不再逐项显示库存文字。当允许两颗融合 +1 时，自动显示升级后的相关配方与材料颜色。本轮直接合成保留独立入口；隐藏配方只计本轮候选。详见[预留提示与计数口径](docs/recipe-reservations.md)。
